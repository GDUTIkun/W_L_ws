...
- MuJoCo：3.7.0 基础模型、canonical Adapter 和 fixed/floating headless runner 已通过 Phase 04；物理参数与控制效果尚未验证。
...
README 与工作流骨架属于仓库引导建设，不作为产品开发 Phase。Phase 05 的拆机执行器辨识仍为当前 active 工作；它可使用 Phase 04 的 MuJoCo 接口，但参数结论仍必须由真实台架证据关闭。
