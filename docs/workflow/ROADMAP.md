...
| 36 | Hip-common-safe rolling realization repair | review | [Phase 46](phases/46-hip-common-safe-rolling-realization-repair/REVIEW.md) | REWORK：corrected exact-R1 fixed-state AUTH 为 `B-HARMFUL-CROSS-REMAINS`；R1 exact、regime 与 convergence 均稳定，但 actual cross `-0.118040` 超 absolute gate，slip self `-0.140169` 反号；已停在 AUTH，下一步仅 post-corrected-R1 authority attribution |
