...
| 14 | nominal NMPC | blocked | [Phase 23](phases/23-nominal-nmpc/PLAN.md) | 用户于2026-08-28冻结；12-state locked-composite model与DG23-01已PASS，OCP/solver/Core/formal均未执行，恢复时从P23-T04继续 |
...
README 与工作流骨架属于仓库引导建设，不作为产品开发 Phase。Phase 14/15/16/17/18/19/20/21/22 已完成；Phase 19最终authority为formal-v4，Phase 20最终authority为formal-v3，Phase 21最终authority为formal-v1，Phase 22最终authority为formal-v2，既往REWORK与formal演进证据均已非覆盖归档。[Phase 23](phases/23-nominal-nmpc/PLAN.md) 已按用户指令冻结为blocked：12-state locked-composite model与DG23-01已关闭，但OCP/solver、production Core、formal与NMPC claims均未获批准，恢复时从P23-T04继续。Phase 05 因当前真机冻结而blocked；恢复时现有PASS不替代通信、Load Cell、同步和安全放行条件。
