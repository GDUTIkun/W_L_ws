...
| 36 | Hip-common-safe rolling realization repair | review | [Phase 46](phases/46-hip-common-safe-rolling-realization-repair/REVIEW.md) | REWORK：equality operator audit 为 `D-QP-CONSTRAINED-REDUCTION/REACTION-MISMATCH`；geometry/J/Jdotv PASS，target effect仅 `1.32%`，QP reaction `99.923%` 不在 equality row-force space且不匹配 coupled rigid solve；overall E 保持，R2 not authorized，下一步可定义一个 Phase46 REWORK repair candidate |
