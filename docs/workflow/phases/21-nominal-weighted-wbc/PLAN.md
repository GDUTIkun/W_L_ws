...
- **Hard constraints：** reduced rigid-body dynamics为12行等式；六路canonical torque box、每轮37-row contact-centred H-cone和12路componentwise acceleration box组成92行不等式，总行数固定由block counts推导为104。当前没有已冻结的state-dependent joint-position/velocity protection；该项保留为P21-T05 decision gate。hard residual超限的candidate无论solver状态为何都不可接受。
...
- **DG21-04 / REWORK / EVIDENCE — equilibrium/hard feasibility：** 42D whole-hard corpus已在P21-T04的4个workspace与28个dynamic problems中证明数学可行并通过solver gate，但这不能替代P21-T05要求的equilibrium起点和逐层`dynamics→torque→H-cone→acceleration`margin/active-row归因，也尚未决定state-dependent joint protection。旧36D layered结果仅属历史single-force合同；完成新42D layered evidence前保持REWORK。
...
| P21-T05 | 逐层验证hard constraints与equilibrium | P21-T03/T04、Phase20 equilibrium | equality-only、torque、H-cone、acceleration及state-dependent joint-protection decision分层结果与margins | DG21-04；旧36D结果仅历史 | todo |
...
**Current blocker：** DG21-01/02/03与P21-T03/T04已关闭；42D变量、104-row hard contract、scale、独立oracle和dense ADMM solver gate已冻结。DG21-04仍为REWORK，下一步仅允许P21-T05逐层执行equilibrium/dynamics→torque→H-cone→acceleration并作state-dependent joint protection decision；P21-T06 task tuning、10 s nonlinear holdout和production model/Core integration继续blocked，旧36D结果不得继承。
