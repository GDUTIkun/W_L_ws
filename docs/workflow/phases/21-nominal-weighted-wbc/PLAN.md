...
- **DG21-03 / CLOSED / CODEX+EVIDENCE — solver：** project-owned Eigen-only fixed dense ADMM已重冻结为42变量/128-row capacity，并以32个独立oracle问题完成1000次cold、repeated-same warm与cycling-dynamic warm审计。三类最大oracle差不超过`3.70871e-7`，最大hard residual不超过`8.71323e-8`，reference-host cold/dynamic最大total setup+solve为`1.455389/1.556428 ms`；20项component tests零失败。36D结果仅保留历史，证据见`evidence/hard_qp_42d.md`与`evidence/solver_audit_42d.md`。
- **DG21-04 / CLOSED / CODEX+EVIDENCE — equilibrium/hard feasibility：** 4个workspace与28个dynamic problems在`dynamics→torque→H-cone→acceleration`四个累积层全部可行；128个QP的最大bound/stationarity/complementarity为`5.47e-14/1.32e-13/2.62e-17`，physical dynamics最大残差`6.50e-13`，三类hard violation与slack均为零。Phase20 equilibrium追加12行`nudot=0`后116-row static problem可行，minimum cone/torque margin为`0.310102/1.99854 N·m`。唯一active attribution为`dynamic_tick_271/right_cone_8`。current model六个active joints均无authoritative limits，故state-dependent joint protection在本profile明确排除而非臆造；证据见`evidence/hard_feasibility_42d.md`。
...
| P21-T06 | 冻结weighted standing tasks与wrench/slack | P21-T05、Phase20 case envelope、tuning/holdout split | task definitions/scales/weights、equilibrium wrench、slack contract、ablation/attribution与formal candidate profile | DG21-05；逐task启停方向、竞争、slack、10 s nonlinear tuning/holdout PASS | todo |
...
**Current blocker：** DG21-01/02/03/04与P21-T03/T04/T05已关闭；42D变量、104-row hard contract、scale、独立oracle、dense ADMM solver gate、逐层hard feasibility和zero-acceleration equilibrium均已冻结。下一步仅允许P21-T06冻结weighted standing tasks、wrench/slack和tuning/holdout nonlinear evidence；production model/Core integration仍须等待DG21-05，旧36D task/nonlinear结果不得继承。
