...
- **DG21-01 / REWORK / CODE+EVIDENCE — reduced model：** 12D state/passive reconstruction、continuous contact differential map和每轮三行soft Pfaffian仍冻结；新的zero-velocity/zero-acceleration static gate在三种等价contact表示中均仅`122/173`可行，51个失败不是condensation或solver假象，故不放行hard-QP。
...
**Current blocker：** continuous六点support的固定contact-centered 6D wrench cone已关闭，但新的static reduced-dynamics gate在condensed-H、V-ray和direct point-force三条路径中均仅`122/173`可行。下一步仅允许继续P21-T03，对51个static failure做torque/contact/dynamics bounded attribution；P21-T04/T05/T06、42D hard-QP、solver benchmark及production model/Core integration继续停止。
