...
- **Decision vector/slack：** 历史36D order为`z=[nudot_12,tau_6,lambda_left3,lambda_right3,slack_left6,slack_right6]`，不再是当前production freeze。slack符号仍固定为`W_feasible = W_reference + slack`；任何新contact representation必须重新冻结完整variable/row order并重跑hard-QP与solver gates。
...
- **DG21-01 / REWORK / CODE+EVIDENCE — reduced model：** 12D state/passive reconstruction、continuous contact differential map和每轮三行soft Pfaffian仍冻结；新的zero-velocity/zero-acceleration static gate在三种等价contact表示中均为raw `122/173`。workspace audit将44个失败归为冻结envelope外的fail-closed coverage，envelope内为`122/129`、仍有7个blocker；7个均由移除contact cone修复、移除torque bounds无效，故定位到base-equilibrium/contact compatibility并继续禁止hard-QP。
- **DG21-02 / CLOSED / CODE+EVIDENCE — contact/wrench map：** continuous六点surface patch在analytic contact center/contact frame中形成固定`G_C:18→6`，24-ray V-cone与37-row H-cone经point/V/H三向1,240-case oracle一致，original `537/537`、fresh `540/540`及transform/virtual-work全部通过；当前冻结contract不依赖point load，允许消除每轮12D internal-force nullspace。该关闭不覆盖DG21-01 static失败或授权QP。
- **DG21-03 / REWORK / CODEX+EVIDENCE — solver：** project-owned Eigen-only fixed dense ADMM的36变量golden/fault/1000次benchmark仅保留为历史结果；当前contact/QP维度未冻结，不能据此关闭solver gate。
- **DG21-04 / REWORK / EVIDENCE — equilibrium/hard feasibility：** QP prefreeze-v5的36D hard-feasibility结果仅属历史single-force合同；任何新contact representation必须重新执行完整hard-QP oracle。
...
| P21-T04 | 冻结QP数学与solver路径 | P21-T03、historical36D contract、candidate solvers | exact variable/matrix/scale spec、solver audit/benchmark、golden QP与failure corpus | DG21-03；旧36D结果仅历史；等待新contact representation | blocked |
| P21-T05 | 逐层验证hard constraints与equilibrium | P21-T03/T04、Phase20 equilibrium | equality-only、torque、normal/friction、joint-protection分层结果与margins | DG21-04；旧36D结果仅历史；等待P21-T03/T04 | blocked |
...
**Current blocker：** continuous六点support的固定contact-centered 6D wrench cone已关闭，但static reduced-dynamics在冻结workspace内仍有7个失败（`122/129`）。bounded attribution排除torque capacity和solver，指向base-equilibrium/contact compatibility；44个workspace外失败仅作为fail-closed coverage。下一步仅允许继续P21-T03，围绕这7个状态核对/修复reduced base equilibrium与冻结contact contract的一致性；P21-T04/T05/T06、42D hard-QP、solver/task/friction调参及production model/Core integration继续停止。
