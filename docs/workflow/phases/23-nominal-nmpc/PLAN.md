...
- **DG23-02 / OPEN / CODEX+EVIDENCE — acados toolchain/OCP/solver/timing：** 冻结`.venv` generation依赖、acados prefix/commit/build flags/library hashes、renderer来源/版本/hash、generated ABI/artifact layout、loader/deployment、OCP transcription、SQP-RTI phase、HPIPM/condensing/regularization/tolerance、warm/reset、status/KKT schema、clean regeneration、1000-run determinism和更新tick组合`<10 ms`。失败则REWORK candidate，不引入silent fallback。
...
| P23-T04 | 冻结acados toolchain、OCP/codegen与solver profile | P23-T03、`/home/t/opt/acados`、SQP-RTI/HPIPM candidate | dependency/loader audit、exact OCP/RTI/HPIPM spec、generated ABI/layout/provenance、prototype、golden/failure corpus与1000-run benchmark | DG23-02；clean generation/build/run、parity、KKT、warm/reset、determinism、组合deadline PASS | doing |
| P23-T05 | 冻结reference/cost/constraint profile | P23-T04、Phase22 envelope、预声明tuning/holdout cases | versioned weights/bounds/reference、delta-wrench transcription、ablation/attribution、tracking/recovery/fault thresholds | DG23-03；tuning后冻结，未见holdout与nonlinear pre-freeze全部PASS | todo |
...
None at PLAN level. 当前执行入口是P23-T04；在DG23-02关闭前，CasADi/acados_template generation环境、shared-library loader、OCP/HPIPM profile、generated artifact和组合deadline均为未批准gate，不得进入production Core或formal。
