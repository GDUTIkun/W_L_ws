...
wheel_leg::RobotState stateFrom(const Row &row) {
  ...
  for (int index = 0; index < 4; ++index) state.q_n_from_b[index] = number(row, "quat" + std::to_string(index));
...
wheel_leg::WbcReference referenceFrom(const Row &row) {
  ...
  for (int index = 0; index < 12; ++index) {
    reference.interaction_wrench_flu[index] = number(row, "reference" + std::to_string(9 + index));
  }
  return reference;
}
...
wheel_leg::WeightedWbcController::Result solveAtTick(
    ...
    const Eigen::Vector2d &longitudinal_acceleration) {
  ...
  for (const auto &row : rows) {
    ...
    if (tick == target_tick) {
      reference.wheel_longitudinal_acceleration_m_s2 = longitudinal_acceleration;
    }
    result = controller.step(stateFrom(row), reference);
    if (tick == target_tick) { found = true; break; }
  }
  if (!found) throw std::runtime_error("tick not found");
  return result;
}

void writeResult(int tick, const std::string &channel, double scale, double sign,
  ...
  for (const double value : result.wheel_position_b_x_m) std::cout << ',' << value;
  for (const double value : result.wheel_velocity_b_x_m_s) std::cout << ',' << value;
  for (const double value : result.wheel_longitudinal_acceleration_m_s2) std::cout << ',' << value;
  for (const double value : result.torque_nm) std::cout << ',' << value;
  for (const double value : result.realized_interaction_wrench_flu) std::cout << ',' << value;
  for (const double value : result.signed_interaction_slack_flu) std::cout << ',' << value;
  std::cout << '\n';
}
        ...
        for (const double scale : {1.0, 0.5}) {
          for (const double sign : {-1.0, 1.0}) {
            writeResult(tick, name, scale, sign,
                        solveAtTick(rows, tick, sign * scale * delta * direction));
          }
        }
      }
    }
